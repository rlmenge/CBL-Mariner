...
%ifarch x86_64
./install-for-efi/usr/bin/grub2-pubkey-mkimage -d ./install-for-efi/usr/lib/grub/x86_64-efi/ --pubkey=%{SOURCE1} -o %{buildroot}%{_datadir}/grub2-efi/grubx64.efi -p /boot/grub2 -O x86_64-efi fat iso9660 part_gpt part_msdos normal boot linux configfile loopback chain efifwsetup efi_gop efi_uga ls search search_label search_fs_uuid search_fs_file gfxterm gfxterm_background gfxterm_menu test all_video loadenv exfat ext2 udf halt gfxmenu png tga lsefi help probe echo lvm cryptodisk luks gcry_rijndael gcry_sha256 gcry_sha512 verify gcry_rsa password_pbkdf2 gcry_dsa
%endif
%ifarch aarch64
./install-for-efi/usr/bin/grub2-pubkey-mkimage -d ./install-for-efi/usr/lib/grub/arm64-efi/ --pubkey=%{SOURCE1} -o %{buildroot}%{_datadir}/grub2-efi/grubaa64.efi -p /boot/grub2 -O arm64-efi fat iso9660 part_gpt part_msdos normal boot linux configfile loopback chain efifwsetup efi_gop ls search search_label search_fs_uuid search_fs_file gfxterm gfxterm_background gfxterm_menu test all_video loadenv exfat ext2 udf halt gfxmenu png tga lsefi help probe echo lvm cryptodisk luks gcry_rijndael gcry_sha256 gcry_sha512 verify gcry_rsa password_pbkdf2 gcry_dsa
%endif
...
%changelog
* Tue Mar 16 2021 Rachel Menge <rachelmenge@microsoft.com> - 2.02-27
- Add grub with pubkey
